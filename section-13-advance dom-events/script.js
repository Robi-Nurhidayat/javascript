...
const headerCallbackObserver = function (entries) {
  const [entry] = entries;
...
const headerObserver = new IntersectionObserver(headerCallbackObserver, {
  root: null,
  threshold: 0,
...
// Reveal Section -> menampilkan section
...
const goToSlide = function (currSlide) {
  slides.forEach(function (s, i) {
    return (s.style.transform = `translateX(${100 * (i - currSlide)}%)`);
  });
};
